...
const EMPTY: UsaForm = {
  empresa: "", nit: "", nombre: "", cedula: "", programa: "", nemonico: "",
  cohorte: "", fecha_inicio: "", email: "", pais: "", direccion: "", ciudad: "",
  telefono: "", valor: "", descuento_pct: "0", fecha_limite_pago: "", observaciones: "",
};
...
export function FacturaUsaForm({ editId }: { editId?: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setForm({
        ...
        nemonico: (data as { nemonico?: string | null }).nemonico ?? "",
        cohorte: data.cohorte ?? "",
        fecha_inicio: data.fecha_inicio ?? "",
        email: data.email ?? "",
        pais: (data as { pais?: string | null }).pais ?? "",
        direccion: (data as { direccion?: string | null }).direccion ?? "",
        ciudad: (data as { ciudad?: string | null }).ciudad ?? "",
        telefono: (data as { telefono?: string | null }).telefono ?? "",
  ...
  const { valorNum, descuentoPct, descuentoFlat, computedTotal } = useMemo(() => {
    ...
    return { valorNum: v, descuentoPct: pct, descuentoFlat: flat, computedTotal: Math.max(v - flat, 0) };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Section title="Datos de la empresa / cliente">
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Empresa *">
            <Input required maxLength={200} value={form.empresa} onChange={(e) => update("empresa", e.target.value)} />
          </Field>
          <Field label="NIT *">
            <Input required maxLength={50} value={form.nit} onChange={(e) => update("nit", e.target.value)} />
          </Field>
