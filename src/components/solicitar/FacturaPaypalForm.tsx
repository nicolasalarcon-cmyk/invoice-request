...
export function FacturaPaypalForm({ editId, duplicateFromId }: { editId?: string; duplicateFromId?: string }) {
  ...
  // campo de valor ES el valor parcial a facturar (aquí ya no aplica descuento
  // en la rama cerrada, así que no hay nada más que sobreescribir).
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalTotalCerrada = (isPartialActive && !isCarteraPartial) ? (Number(form.valor_parcial) || 0) : valorNum;
      ...
      const payload = {
        ...
        descuento: isJuridica ? (valorPorEstudianteNum - valorTotalPorEstudiante) : 0,
        descuento_pct: isJuridica ? (Number(form.descuento_pct) || 0) : 0,
        descuento_bono: isJuridica ? (valorPorEstudianteNum - valorTotalPorEstudiante) : 0,
        ...
        valor_parcial: isPartialActive ? (isCarteraPartial ? (isJuridica ? valorTotalEmpresa : valorNum) : Number(form.valor_parcial)) : null,
        ...
        <>
          ...
          <Section title="Datos de la factura">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {isJuridica ? (
                <>
                  <Field label={isCarteraPartial ? "Valor Parcial por Estudiante *" : "Valor por Estudiante *"}>
                ...
                <>
                  <Field label={isCarteraPartial ? "Valor Parcial a Pagar *" : "Valor *"}>
