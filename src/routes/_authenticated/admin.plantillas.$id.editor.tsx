...
export const Route = createFileRoute("/_authenticated/admin/plantillas/$id/editor")({
  component: EditorPage,
  head: () => ({ meta: [{ title: "Editor de plantilla · Admin" }] }),
});
...
function EditorPage() {
  ...
  // Generate PDF preview (client-side only)
  const generatePreview = useCallback(async (
    ...
    try {
      ...
      if (docType === "factura_usa") {
        ...
        element = React.createElement(FacturaUSADocument, {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          data: USA_SAMPLE as any,
          _tpl: t,
        });
      } else {
  ...
  // Initial preview when template loads — auto-select tab from template's document type
  useEffect(() => {
    if (!savedTpl) return;
  ...
  return (
    <div className="flex flex-col h-screen bg-background overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 border-b border-border flex-shrink-0 bg-card">
        <Button variant="ghost" size="sm" onClick={() => navigate({ to: "/admin/plantillas" })}>
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => generatePreview(editing, previewDocType)}
          ...
          <div className="p-5 space-y-7">
            ...
            <Section title="Datos de la institución">
              <Field label="Nombre de la institución">
                <Input
                  value={editing.institucion_nombre}
              ...
              <Field label="Descripción legal (aparece en cabecera del PDF)">
                <Textarea
                  rows={3}
                  value={editing.descripcion_legal}
                  onChange={(e) => update("descripcion_legal", e.target.value)}
                  className="text-xs"
                />
              </Field>
            </Section>

            {/* Pie de página — most important section */}
            <Section title="Pie de página">
              ...
              <div className="space-y-1">
                <Field label="Medios de pago (texto en negrita)">
                  <Textarea
                    rows={3}
                    value={editing.medios_pago}
                    onChange={(e) => update("medios_pago", e.target.value)}
                    className="font-mono text-xs"
                    placeholder="Bancolombia Cuenta Ahorros 16869342576 a nombre de…"
                  />
                </Field>
                {editing.medios_pago && (
                  <p className="text-right text-xs text-muted-foreground">{editing.medios_pago.length} caracteres</p>
                )}
              </div>
              ...
              <div className="space-y-1">
                <Field label="Nota sobre retención en la fuente">
                  <Textarea
                    rows={3}
                    value={editing.nota_retencion}
                    onChange={(e) => update("nota_retencion", e.target.value)}
                    className="font-mono text-xs"
                    placeholder="Favor NO HACER RETENCIÓN EN LA FUENTE…"
                  />
                </Field>
                {editing.nota_retencion && (
                  <p className="text-right text-xs text-muted-foreground">{editing.nota_retencion.length} caracteres</p>
                )}
              </div>
              ...
              <div className="space-y-1">
                <Field label="Nota legal final">
                  <Textarea
                    rows={3}
                    value={editing.nota_legal}
                    onChange={(e) => update("nota_legal", e.target.value)}
                    className="font-mono text-xs"
                    placeholder="Régimen tributario especial. Esta factura se asimila…"
                  />
                </Field>
                {editing.nota_legal && (
                  <p className="text-right text-xs text-muted-foreground">{editing.nota_legal.length} caracteres</p>
                )}
              </div>
            </Section>
                ...
                <div>
                  <Field label="Recargo (%)">
                    <Input
                      type="number"
                      min={0}
                      value={editing.recargo_pct}
                      onChange={(e) => update("recargo_pct", Number(e.target.value))}
                    />
                  </Field>
                ...
                <div>
                  <Field label="Días límite">
                    <Input
                      type="number"
                      min={0}
                      value={editing.dias_limite}
                      onChange={(e) => update("dias_limite", Number(e.target.value))}
                    />
                  </Field>
                ...
                <div>
                  <Field label="Días extra">
                    <Input
                      type="number"
                      min={0}
                      value={editing.dias_extraordinario}
                      onChange={(e) => update("dias_extraordinario", Number(e.target.value))}
                    />
                  </Field>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-4">
      <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground border-b border-border pb-1.5">
        {title}
      </h2>
      {children}
    </div>
