...
import {
  adminListUsers, adminCreateUser, adminUpdatePassword, adminUpdateRole, adminDeleteUser,
...
function UsersPage() {
  ...
  const updatePw = useServerFn(adminUpdatePassword);
  ...
  const load = async () => {
    setLoadingList(true);
    try { setUsers(await list()); } catch (e) { toast.error(e instanceof Error ? e.message : "Error"); }
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.success("Usuario creado y notificado por correo");
      setCreating(false);
  ...
  const handlePw = async () => {
    ...
    try {
      await updatePw({ data: { userId: pwUser.id, password: newPw } });
